...
import { ScrollArea, StickyNoteStack, type CandidateRound } from '@kibotalk/ui'
...
/**
 * Newest round on top. Sticky yellow (shared `StickyNoteStack`) in floating
 * surface; window uses playground-only flat paper cards for dev comparison.
 */
export function CandidateRoundStack({
  ...
  if (surface === 'floating') {
    return <StickyNoteStack rounds={rounds} maxRounds={maxRounds} streaming={streaming} emptyHint={emptyHint} />
  }

  return (
    <ScrollArea className="h-[min(72vh,40rem)] w-full" viewportRef={viewportRef} viewportClassName="px-2 pb-6 pt-4">
          ...
          {visible.map((round, roundIndex) => {
            ...
            return (
              ...
              >
                ...
                <WindowRoundCard candidates={round.candidates} label={isLatest ? '本轮建议' : undefined} />
              </motion.div>
            )
          })}
        </AnimatePresence>
